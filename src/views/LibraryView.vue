...
function chatWithCharacter(id: string) {
  router.push(`/chat/${id}`)
}
...
<template>
  <div class="min-h-screen bg-[var(--bg)] text-[var(--text)] p-5 animate-slide-up">
    <!-- Header -->
    <div class="flex flex-wrap items-center gap-3 mb-8">
      ...
      <button @click="router.push('/novel-to-worldbook')" class="btn-secondary">📚 txt转世界书</button>
    ...
    <!-- Empty state -->
    <div v-else-if="!filtered.length" class="max-w-md mx-auto text-center py-16 px-6 glass-panel rounded-2xl border border-white/5 my-10 animate-slide-up">
    ...
    <!-- Character grid -->
    <div v-else class="grid grid-cols-[repeat(auto-fill,minmax(185px,1fr))] gap-5">
      <div
      ...
      >
        ...
        <div class="aspect-[3/4] relative overflow-hidden bg-zinc-950">
          ...
          <div class="absolute inset-0 bg-gradient-to-t from-black/90 via-black/40 to-transparent opacity-0 group-hover:opacity-100 transition-all duration-300 flex flex-col justify-end p-3 gap-2 translate-y-3 group-hover:translate-y-0">
            <button @click="chatWithCharacter(char.id)" class="btn-sm-primary w-full py-1.5 font-semibold text-xs rounded-lg shadow-lg shadow-purple-500/20 flex items-center justify-center gap-1">💬 聊天</button>
          ...
          <button
            ...
            :class="char.isFavorite ? 'text-yellow-400 hover:text-yellow-300' : 'text-white/40 hover:text-white'"
          >★</button>
        </div>
        <!-- Name + tags -->
        <div class="p-3">
        ...
        <button
          ...
          class="absolute top-2 left-2 w-7 h-7 rounded-full bg-black/50 border border-white/10 hover:bg-red-600 hover:border-red-500 flex items-center justify-center text-white/50 hover:text-white text-xs opacity-0 group-hover:opacity-100 transition-all duration-200 backdrop-blur-sm cursor-pointer"
          title="删除角色"
        >✕</button>
      </div>
    </div>
